...
- **Session log**: `.vibe-coder-log.json` in the workspace (dot-prefixed → skipped by `collectPaths`, so it never enters LLM context) — a JSON array of per-session summaries (questions, files written, failure counts) written after every turn via the same delete+re-add pattern, for autograder consumption. Logging is wrapped so it can never break the coach. This is the only record of student questions anywhere: Codio's platform coach-log export logs only `userPrompt`, which messages-based coaches leave empty (verified in a real export, Aug 2026).
